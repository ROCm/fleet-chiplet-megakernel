    ...
    void const *bias_ptr = nullptr) // [1, full_N] bf16, optional
{
  ...
  float *d_ws =
      reinterpret_cast<float *>(__uniform_addr(workspace_ptr)) +
...
#pragma unroll
  ...
  if (is_last) {
    ...
    bf16 const *res =
        residual_ptr ? reinterpret_cast<bf16 const *>(
                           __uniform_addr(static_cast<T const *>(residual_ptr) +
                                          static_cast<size_t>(n_tile) * tile_n))
                     : nullptr;
    bf16 const *bias = bias_ptr ? reinterpret_cast<bf16 const *>(__uniform_addr(
                                      static_cast<T const *>(bias_ptr) +
                                      static_cast<size_t>(n_tile) * tile_n))
                                : nullptr;
    bf16 *out = reinterpret_cast<bf16 *>(__uniform_addr(
        static_cast<T *>(output_ptr) + static_cast<size_t>(n_tile) * tile_n));
    ...
    for (int idx = threadIdx.x; idx < TOTAL; idx += 256) {
      ...
      if (bias) {
        val += type_convert<float>(bias[col]);
      }

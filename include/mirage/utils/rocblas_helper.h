...
// Define device macros for regular C++ compilation (when not using HIP
// compiler) These are needed when compiling Cython-generated code with regular
// g++
#ifndef __HIP__
#ifndef __device__
#define __device__
...
// Note: __fp16 and __bf16 are only available with HIP/ROCm compilers (hipcc or
// clang with HIP) For regular g++ compilation (like Cython), we use uint16_t as
// storage These will be properly converted when interfacing with rocBLAS APIs
...
// Note: This is fine for host code compilation - device code will use proper
// types when compiled with HIP The uint16_t is just a placeholder for type
...
// Note: __fp16 and __bf16 cannot be used as function parameters or template
// parameters So we use a traits-based approach with wrapper types
template <typename T>
...
// Note: __fp16 and __bf16 cannot be used as template parameters, so we use a
// constexpr function that checks type properties at compile time
template <typename T>
constexpr rocblas_datatype get_rocblas_type_impl() {
  if constexpr (std::is_same_v<T, float>) {
    ...
    if constexpr (std::is_same_v<T, uint16_t>) {
      // This is our fallback type for half_t/bfloat16_t when not using HIP
      // compiler Treat it as half precision
...
template <typename ElementA,
          ...
          typename ElementC,
          typename ElementD>
rocblas_status gemm(rocblas_handle handle,

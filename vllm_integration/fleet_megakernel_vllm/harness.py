...
and text. Enable fleet_mk via `VLLM_PLUGINS=fleet_mk`; disable (stock baseline) via
`VLLM_PLUGINS=` (empty). Compare the two token streams for parity.

Fleet MK's zero-copy KV path requires an aiter flash attention backend
(reshape_and_cache_flash, non-reordered) and block_size == fleet_mk PAGE_SIZE; the
harness reads PAGE_SIZE from the model's spec and sets both when the plugin is on.
Override via FLEET_MK_BLOCK_SIZE. FLEET_MK_TEMP / FLEET_MK_SEED drive vLLM's sampler.
FLEET_MK_MODEL picks the model (dense Qwen3 or MoE GPT-OSS).

Usage:
  VLLM_PLUGINS=       python -m fleet_megakernel_vllm.harness   # stock baseline
  VLLM_PLUGINS=fleet_mk  python -m fleet_megakernel_vllm.harness   # fleet_mk decode (aiter)
"""
...
# on VLLM_PLUGINS) because the stock baseline must run the SAME backend as fleet_mk or
...
def _custom_attention_backend():
    ...
    stock aiter backends already allocate fleet_mk's split KV layout, so there is
...
def main():
    ...
    max_tokens = int(os.environ.get("FLEET_MK_MAX_TOKENS", "1000"))
    model = os.environ.get("FLEET_MK_MODEL", "Qwen/Qwen3-8B")
    temperature = float(os.environ.get("FLEET_MK_TEMP", "0.0"))
    seed = int(os.environ.get("FLEET_MK_SEED", "0"))
    ...
    llm_kwargs = dict(
        ...
        # fleet_mk bug and is not one. Overridable so a quiet GPU can still use 0.9.
    ...
    if fleet_mk_on:
        ...
        # forces the V0 engine). block_size == PAGE_SIZE so 1 vLLM block == 1 fleet_mk
        # page (enables zero-copy KV aliasing).
        #
        # Use the aiter UNIFIED-ATTENTION backend (Triton unified_attention), not
        # the MHA/flash one. Both write KV via reshape_and_cache_flash in fleet_mk's
        ...
        # block_size == PAGE_SIZE so 1 vLLM block == 1 fleet_mk page (zero-copy KV).
        # PAGE_SIZE is now 16 -- vLLM's own ROCm default -- so this override is
        # a no-op in practice and fleet_mk binds to a stock-configured engine. It
        ...
        # physical KV memory with SEPARATE per-group block tables. fleet_mk's zero-copy
        ...
        # table, one KV tensor per layer. fleet_mk's own kernel still applies the
        ...
        # (num_blocks, n_kv, block_size, 2*head_size), which fleet_mk's flat view
        # cannot alias. plugin.register() registers FleetMKAttentionBackend under
    ...
    gen = out[0].outputs[0]
    label = "fleet_mk" if os.environ.get("VLLM_PLUGINS") else "stock"

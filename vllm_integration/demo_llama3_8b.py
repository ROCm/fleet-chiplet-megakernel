...
Fleet MK demo for llama3-8b inference using two-level fusion.

Loads llama3-8b weights from HuggingFace, quantizes to MXFP4, builds the
pointer table for the llama3-8b persistent kernel, runs prefill via PyTorch,
then decode via the fleet_mk kernel.

Usage:
    HIP_VISIBLE_DEVICES=0 python3 demo_llama3_8b.py \
...
VOCAB_SIZE = 128256
PADDED_VOCAB_SIZE = 128512  # next multiple for LM head split
NUM_LAYERS = 32
NUM_Q_HEADS = 32
NUM_KV_HEADS = 8
HEAD_DIM = 128
...
GATEUP_OUTPUT_SIZE = 2 * INTERMEDIATE_SIZE  # 28672
GATEUP_OPW = 128
GATEUP_N_WGS_PER_XCD = (GATEUP_OUTPUT_SIZE // GATEUP_OPW) // NUM_XCDS  # 28
DOWN_N_WGS_PER_XCD = (HIDDEN_SIZE // OUTPUT_PER_WG) // NUM_XCDS  # 8
LM_N_WGS_PER_XCD = (PADDED_VOCAB_SIZE // OUTPUT_PER_WG) // NUM_XCDS  # 251
...
# Pointer table layout (matches llama3_8b_kernel.cuh)
PTRS_IN = 16
PTRS_OUT = 9
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fleet MK llama3-8b demo")
    parser.add_argument("--model", default="Llama3/LLAMA3-8B",
    ...
    parser.add_argument("--fleet-mk-so", default=None,
                        help="Path to compiled llama3_8b.so")
    parser.add_argument("--pytorch-only", action="store_true",
                        help="Use PyTorch for decode (skip fleet_mk kernel)")
    return parser.parse_args()


def load_llama3_8b_kernel(so_path: str):
    """Load the compiled Llama3 fleet_mk .so."""
    lib = ctypes.CDLL(so_path)

    lib.llama3_8b_init.restype = None
    lib.llama3_8b_init.argtypes = []

    lib.llama3_8b_launch.restype = None
    lib.llama3_8b_launch.argtypes = [
    ...
    lib.llama3_8b_finalize.restype = None
    lib.llama3_8b_finalize.argtypes = []
...
def main():
    ...
    print(f"Loading llama3-8b from: {model_path}")
    ...
    print(f"llama3-8b: {NUM_LAYERS} layers, hidden={HIDDEN_SIZE}, "
    ...
    for xcd in range(NUM_XCDS):
        for li in range(NUM_LAYERS):
            ...
            # input_ptrs[16] - matches llama3_8b_ptr namespace (Dense)
            input_ptrs = [
            ...
            # output_ptrs[9] - matches llama3_8b_ptr namespace (Dense)
            output_ptrs = [
    ...
    fleet_mk_so = args.fleet_mk_so or os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "generated", "llama3_8b.so")
    print(f"Loading fleet_mk kernel from: {fleet_mk_so}")
    fleet_mk = load_llama3_8b_kernel(fleet_mk_so)
    fleet_mk.llama3_8b_init()
    ...
    if os.environ.get("FLEET_MK_SUBPHASE_TIMING"):
        ...
        if valid_layers > 0:
            ...
            print(f"    {'TOTAL x 32 layers':62s}: {total_avg * 32 / 1000.0:8.3f} ms")

    fleet_mk.llama3_8b_finalize()
    print("\nDone.")

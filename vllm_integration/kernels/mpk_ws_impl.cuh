/* Fleet MK: real implementations of fleet's worker-state tracing macros.
 *
 * Fleet's task headers are already instrumented for exactly the failure fleet_mk
 ...
 *
 * That is the entire question for fleet_mk's layer-0 hang, and answering it by
 * bisecting layers costs a 4-minute build-and-run per bit. Fleet defines these
 * macros in persistent_kernel.cuh, which fleet_mk does not compile; fleet_mk's
 ...
 * checks out. Enabled only by -DFLEET_MK_WORKER_STATE.
 */
#pragma once

#ifdef FLEET_MK_WORKER_STATE
...
__device__ __forceinline__ void
    fleet_mk_ws_phase(int phase, int layer, int xcd, int tid) {
...
__device__ __forceinline__ void
    fleet_mk_ws_wait_begin(int barrier_id, int expected, int tid) {
  if (tid == 0 && g_ws_dev != nullptr) {
...
__device__ __forceinline__ void
    fleet_mk_ws_wait_tick(int observed, int spins, int tid) {
  if (tid == 0 && g_ws_dev != nullptr) {
    int *b = g_ws_dev + g_ws_nworkers * 4 + blockIdx.x * 4;
...
__device__ __forceinline__ void
    fleet_mk_ws_wait_aux(int a0, int a1, int tid) {
  if (tid == 0 && g_ws_dev != nullptr) {
...
__device__ __forceinline__ void fleet_mk_ws_mark(int code, int aux, int tid) {
  if (tid == 0 && g_ws_dev != nullptr) {
    int *b = g_ws_dev + g_ws_nworkers * 4 + blockIdx.x * 4;
    __atomic_store_n(
...
__device__ __forceinline__ void fleet_mk_ws_wave_exit(int wave, int tid) {
...
__device__ __forceinline__ void fleet_mk_ws_wave_sync(int wave, int tid) {
  if ((tid & 63) == 0 && g_ws_dev != nullptr) {
    int *b = g_ws_dev + g_ws_nworkers * 8 + blockIdx.x * 4;
...
__device__ __forceinline__ void fleet_mk_ws_wave_clear(int wave, int tid) {
  if ((tid & 63) == 0 && g_ws_dev != nullptr) {
    int *b = g_ws_dev + g_ws_nworkers * 8 + blockIdx.x * 4;
...
#define MPK_WS_PHASE(phase, layer, xcd) fleet_mk_ws_phase((phase), (layer), (xcd), tid)
#define MPK_WS_WAIT_BEGIN(barrier_id, expected)                                \
  fleet_mk_ws_wait_begin((barrier_id), (expected), tid)
#define MPK_WS_WAIT_TICK(observed, spins)                                      \
  do {                                                                         \
    if (((spins) & (MPK_WS_WAIT_REFRESH - 1)) == 0) {                          \
      fleet_mk_ws_wait_tick((observed), (spins), tid);                            \
    }                                                                          \
  } while (0)
#define MPK_WS_WAIT_AUX(a0, a1, a2, a3) fleet_mk_ws_wait_aux((a0), (a1), tid)
#define MPK_WS_MARK(code, aux) fleet_mk_ws_mark((code), (aux), tid)
#define MPK_WS_WAVE_EXIT(wave) fleet_mk_ws_wave_exit((wave), tid)
#define MPK_WS_WAVE_SYNC(wave) fleet_mk_ws_wave_sync((wave), tid)
#define MPK_WS_WAVE_CLEAR(wave) fleet_mk_ws_wave_clear((wave), tid)

// MPK_WS_ON gates fleet's *inline dump* sites, which read a RuntimeConfig field
// (precomp_dbg_worker_state) that fleet_mk's GptConfig does not have. Leave it
...
#define FLEET_MK_WS_INTS_PER_WORKER 16

#endif // FLEET_MK_WORKER_STATE

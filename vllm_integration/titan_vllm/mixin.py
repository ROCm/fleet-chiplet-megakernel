"""TitanModelMixin: the model-agnostic machinery that runs decode through titan's
...
thin subclasses (model.py) via three hooks:
  ...
  * class attribute `TITAN_ARCH` -- the vLLM architecture name load_spec() keys on.
  * `_titan_layer_attn(li)`       -- the vLLM Attention module for layer li
                                     (dense: layers[li].self_attn.attn;
                                      moe:   layers[li].attn.attn).
  * `_titan_pack_weights(...)`    -- MXFP4-pack this model's weights into titan's
                                     layout (dense 12-slot vs MoE 13-slot).
  * `_titan_embed_tokens(ids)`    -- token embedding lookup (dense: embed_tokens;
                                     moe: embedding); the mixin zero-pads it.

MRO note: subclasses must list the mixin FIRST
(`class T(TitanModelMixin, StockModel)`) so the mixin's forward()/load_weights()
...
from .runtime import TitanBuffers, TitanDecoder, build_ptr_table, load_kernel
...
class TitanModelMixin:
    ...
    def _titan_setup_state(self, vllm_config):
        ...
        self._titan_model_path = vllm_config.model_config.model
        self.titan = None             # TitanDecoder, built on first decode
        self._titan_weight_refs = []  # keep packed tensors alive
        self._titan_kv_bound = False  # aliased vLLM KV + ptr_table built yet?
        self._titan_spec = None

    # ── weight loading + titan setup ─────────────────────────────────────────
    def load_weights(self, weights):
        loaded = super().load_weights(weights)
        self._setup_titan()
        return loaded

    def _setup_titan(self):
        from .packing import import_mirage_packers

        S = load_spec(self.TITAN_ARCH)
        self._titan_spec = S
        dev = next(self.parameters()).device
        packers = import_mirage_packers()

        # Weight-footprint instrumentation. The whole point of sourcing titan's
        ...
        # whatever else is measuring); TITAN_MEM_TRACE=1 to enable.
        mem_trace = os.environ.get("TITAN_MEM_TRACE") == "1"
        if mem_trace:
            torch.cuda.synchronize(dev)
            torch.cuda.reset_peak_memory_stats(dev)
            mem_before = torch.cuda.memory_allocated(dev)
            print(f"[TITAN_MEM] before pack: allocated="
        ...
        packed = self._titan_pack_weights(packers, S, dev)

        if mem_trace:
            torch.cuda.synchronize(dev)
            after = torch.cuda.memory_allocated(dev)
            peak = torch.cuda.max_memory_allocated(dev)
            print(f"[TITAN_MEM] after pack:  allocated={after / 2**30:.3f} GiB"
                  f"  (+{(after - mem_before) / 2**30:.3f} GiB)", flush=True)
            print(f"[TITAN_MEM] peak during pack: "
        ...
        self._titan_weight_refs.extend(packed["weight_refs"])
        ...
        if os.environ.get("TITAN_HASH_PACKED"):
            # tools/ is a sibling of titan_vllm/, not a package on the path when
            ...
            hash_tensors(named, os.environ["TITAN_HASH_PACKED"])
        ...
        self._titan_buffers = TitanBuffers(
        ...
        if packed.get("cos") is not None and packed.get("sin") is not None:
            ...
            self._titan_weight_refs += [self._cos, self._sin]
        else:
    ...
    # ── zero-copy KV: alias vLLM's KV into titan buffers on first decode ──────
    def _ensure_titan_kv_bound(self):
        """Alias titan's per-layer K/V onto vLLM's kv_cache and build the ptr table.
        ...
        so kv_cache[0]/[1] reshape to titan's flat [entries, kv_cache_stride] K/V
        with no copy. Requires block_size == PAGE_SIZE (1 vLLM block == 1 titan
        page).
        """
        if self._titan_kv_bound:
            return
        S = self._titan_spec
        k_aliases, v_aliases = [], []
        for li in range(S.num_layers):
            attn = self._titan_layer_attn(li)
            if li == 0:
                self._assert_flash_backend(attn)
            kvc = self._layer_kv_tensor(attn)
            assert kvc.dim() == 5 and kvc.shape[0] == 2, (
                f"titan zero-copy needs a (2, num_blocks, block_size, n_kv, "
            ...
            assert block_size == S.page_size, (
                f"titan PAGE_SIZE={S.page_size} but vLLM block_size={block_size}; "
            ...
            assert n_kv * head_size == S.kv_cache_stride, (
                ...
                f"!= titan kv_cache_stride {S.kv_cache_stride}.")
            ...
            assert (k_alias.data_ptr() == kvc[0].data_ptr()
                ...
                f"layer {li}: titan KV alias is a copy, not a view of vLLM's "
                f"kv_cache -- zero-copy broken.")
            k_aliases.append(k_alias)
            v_aliases.append(v_alias)
        self._titan_buffers.set_kv_aliases(k_aliases, v_aliases)

        ptr_table = build_ptr_table(S, self._weight_ptrs_host,
                                    self._titan_buffers, self._moe_scale_ptrs)
        self.titan = TitanDecoder(
            S, self._lib, self._titan_buffers, ptr_table, self._cos, self._sin,
            self._final_norm_w, self._lm_head_packed, self._lm_head_bias,
            embed_weight=self._titan_embed_weight() if S.is_moe else None)
        self._titan_kv_bound = True
    ...
    def _layer_kv_tensor(attn):
        ...
        on a new vLLM silently binds titan to 1/num_blocks of the pool at the
        ...
        if isinstance(kvc, (list, tuple)):        # 0.11.x: per virtual engine
            assert len(kvc) == 1, (
                f"titan decode assumes a single virtual engine; got "
        ...
        assert isinstance(kvc, torch.Tensor) and kvc.numel() > 0, (
            ...
            f"bound it yet. _ensure_titan_kv_bound must run on the first real "
    ...
    def _assert_flash_backend(attn):
        ...
        # earlier allocated titan's split layout directly. TitanAttentionBackend
        ...
        ok = {"AiterFlashAttentionBackend", "RocmAiterUnifiedAttentionBackend",
              "TitanAttentionBackend"}
        assert name in ok, (
            f"titan zero-copy KV requires an aiter flash backend "
    ...
    def forward(self, input_ids, positions, intermediate_tensors=None,
        ...
        if S.is_moe:
            assert inputs_embeds is None, (
                "titan MoE decode embeds on-device and cannot accept "
            ...
            if inputs_embeds is not None:
                ...
                embed = self._titan_embed_tokens(input_ids)[0]
        ...
        self.titan.decode_step(embed, cur_pos, block_table=block_table,
                               sync=False, token_id=token_id)
        self._titan_fused_logits = S.is_moe
        # Slice off the MoE padding so the returned width matches vLLM's lm_head.
        hs = self._titan_buffers.buf_lm_norm_scratch[:input_ids.shape[0], :S.hidden_size]
        return hs.clone()

    def compute_logits(self, hidden_states):
        """Return titan's fused logits when the last forward() was a titan decode.
        ...
        weight read (0.200 ms/token measured) for a value titan already had, so
        the fused row is strictly cheaper and, being the same arithmetic on the
        same MXFP4 weights the argmax used, cannot disagree with titan's own
        greedy pick.

        The flag is consumed (not just read) so any path that reaches
        compute_logits without a preceding titan decode -- prefill, a dummy run,
        ...
        """
        if not getattr(self, "_titan_fused_logits", False):
            return super().compute_logits(hidden_states)
        self._titan_fused_logits = False
        S = self._titan_spec
        n = hidden_states.shape[0]
        return self._titan_buffers.buf_logits[:n, :S.vocab_size]
    ...
    def _titan_embed_tokens(self, input_ids):
        ...
        Unused on the MoE path -- see _titan_embed_weight.
        """
        return self._titan_embed_fn()(input_ids)

    def _titan_embed_fn(self):
        ...
        raise AttributeError(
            ...
            f"_titan_embed_tokens.")

    def _titan_embed_weight(self):
        ...
        assert mod is not None and hasattr(mod, "weight"), (
            ...
            f"override _titan_embed_weight (model has: "

...
Titan demo for gpt-oss-120b inference using two-level fusion.
...
# rows at ITS pitch while still reducing 2944. Setting this here packs titan's
...
def parse_args():
    parser = argparse.ArgumentParser(description="Titan gpt-oss-120b demo")
...
def main():
    ...
    page_size = 16  # == titan PAGE_SIZE (vLLM's ROCm default block size)
    ...
    for xcd in range(NUM_XCDS):
        for li in range(NUM_LAYERS):
            ...
            mirage_in = [
                ...
                # reason.
                (next_qkv_weight_xcd if TITAN_QKV_PF else w13_scale_base),   # [24]
                (prefetched_qkv_weight_xcd if TITAN_QKV_PF else w2_scale_base),  # [25]
            ]
    ...
    titan = load_gpt_oss_120b_kernel(titan_so)
    titan.gpt_oss_120b_init()
    ...
    if args.pytorch_only:
        ...
        if use_decode_loop:
            ...
            titan.gpt_oss_120b_decode_loop(
                1, attn_scale,
                cos_padded.data_ptr(), sin_padded.data_ptr(),
                qo_indptr.data_ptr(), kv_indptr.data_ptr(),
                kv_indices.data_ptr(), kv_last_page_len.data_ptr(),
            ...
            titan.gpt_oss_120b_pipe_capture(
                1, attn_scale,
                cos_padded.data_ptr(), sin_padded.data_ptr(),
                qo_indptr.data_ptr(), kv_indptr.data_ptr(),
                kv_indices.data_ptr(), kv_last_page_len.data_ptr(),
            ...
            starter.record()
            titan.gpt_oss_120b_pipe_launch_all(
                stream.cuda_stream,
                remaining,
            )
            ender.record()
            ...
            titan.gpt_oss_120b_pipe_destroy()
            ...
            for di in range(output_len):
                ...
                if use_graph and graph_captured:
                    ...
                    if di >= args.warmup:
                        starter.record()
                    titan.gpt_oss_120b_graph_launch(
                        cur_token,
                        stream.cuda_stream,
                    )
                elif use_graph and di == args.warmup - 1:
                    # Last warmup: capture graph
                    titan.gpt_oss_120b_graph_capture(
                        1, attn_scale,
                        cos_padded.data_ptr(), sin_padded.data_ptr(),
                        qo_indptr.data_ptr(), kv_indptr.data_ptr(),
                        kv_indices.data_ptr(), kv_last_page_len.data_ptr(),
